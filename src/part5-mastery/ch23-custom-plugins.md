...
이 챕터에서는 플러그인의 구조와 규칙을 배우고, 두 개의 실전 플러그인을 처음부터 끝까지 만들어 본다.
...
현대 Neovim 플러그인의 사실상 표준(de facto standard)은 `setup()` 패턴이다. 사용자가 플러그인을 설정할 때 `require('plugin').setup(opts)`를 호출하는 방식이다.
...
사용자가 키맵을 커스터마이징할 수 있도록 옵션으로 제공하는 것이 좋은 관행이다.

## 하이라이트 그룹 설정

플러그인에서 사용하는 색상을 하이라이트 그룹(highlight group)으로 정의한다.
...
-- 매개변수: (버퍼 번호, 시작 줄, 끝 줄, strict_indexing)
-- 0은 현재 버퍼, -1은 마지막 줄까지
...
floating window는 화면 위에 떠 있는 창으로, 팝업(popup) UI를 만들 때 사용한다.

```lua
-- 빈 버퍼 생성
local buf = vim.api.nvim_create_buf(false, true)
-- 매개변수: (listed, scratch)
-- listed=false: 버퍼 목록에 표시 안 함
-- scratch=true: 일회용 버퍼
...
local win = vim.api.nvim_open_win(buf, true, {
  relative = 'editor',       -- 에디터 기준 위치
  width = width,
  height = height,
  col = (vim.o.columns - width) / 2,   -- 가운데 정렬
  row = (vim.o.lines - height) / 2,
  style = 'minimal',        -- 최소한의 UI
  border = 'rounded',       -- 둥근 테두리
})

-- 창 옵션 설정
vim.api.nvim_win_set_option(win, 'winblend', 10)  -- 투명도
...
네임스페이스(namespace)는 플러그인이 추가한 하이라이트, extmark 등을 구분하는 고유 ID다.
...
네임스페이스를 사용하면 다른 플러그인의 하이라이트에 영향을 주지 않고 자신의 하이라이트만 관리할 수 있다.
...
M.config = {
  note_file = '.notes.md',      -- 메모 파일명
  width = 0.6,                  -- 화면 너비의 60%
  height = 0.6,                 -- 화면 높이의 60%
  border = 'rounded',
}
...
plenary.nvim은 Neovim 플러그인 테스트를 위한 프레임워크(testing framework)를 제공한다.
...
-- 테스트에 필요한 최소한의 초기화
vim.opt.rtp:append('.')              -- 현재 플러그인을 runtimepath에 추가
vim.opt.rtp:append('~/.local/share/nvim/lazy/plenary.nvim')  -- plenary 경로
```

## 배포: GitHub에 올리고 lazy.nvim으로 설치하기
...
플러그인을 배포하면 누구나 lazy.nvim으로 설치할 수 있다:
...
1. `~/.config/nvim/lua/wordcount/init.lua` 경로에 단어 수 플러그인을 구현한다
2. `:WordCount` 명령으로 floating window에 통계를 표시한다
3. `q`로 창을 닫는 기능을 추가한다
4. 제목에 하이라이트를 적용한다

**연습 2: 빠른 메모 플러그인 구현**

1. `~/.config/nvim/lua/quicknote/init.lua` 경로에 빠른 메모 플러그인을 구현한다
...
- **`M.setup(opts)`** 패턴이 현대 Neovim 플러그인의 표준이다. `vim.tbl_deep_extend`로 기본값과 사용자 설정을 병합한다
- **`nvim_create_user_command`**로 `:명령어`를 등록하고, **`vim.keymap.set`**으로 키맵을 연결한다
- **버퍼 API**: `nvim_buf_get_lines`/`nvim_buf_set_lines`로 버퍼 내용을 읽고 쓴다
- **Floating window**: `nvim_open_win`으로 팝업 UI를 만든다. `relative`, `border`, `title` 등으로 외형을 제어한다
- **네임스페이스**: `nvim_create_namespace`로 플러그인의 하이라이트를 다른 플러그인과 분리한다
- **테스트**: plenary.nvim의 `describe`/`it` 구문으로 테스트를 작성하고 headless 모드로 실행한다
